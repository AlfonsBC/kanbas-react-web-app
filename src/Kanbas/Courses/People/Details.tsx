...
export default function PeopleDetails({ fetchUsers }:
    ...
    const fetchUser = async () => {
        if (!uid) return;
        const usernew = await client.findUserById(uid);
        setUser(usernew);
        console.log(usernew)
    };
    useEffect(() => {
...
{!editing && (<FaPencil onClick={() => setEditing(true)} className="float-end fs-5 mt-2" /> )}
{editing && (<FaCheck onClick={() => saveUser()} className="float-end fs-5 mt-2 me-2" /> )}
{!editing && (
